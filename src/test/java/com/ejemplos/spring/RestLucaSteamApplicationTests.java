...
class RestLucaSteamApplicationTests {
    ...
    void testListJuegosDevuelveLista() throws Exception {
        ...
        mockMvc.perform(get("/juegos")
                ...
                .andExpect(jsonPath("$[0].nombre").value("capo"));
    }
    ...
    public void testObtenerJuegos() throws Exception {
        ...
        mockMvc.perform(get("/juegos/anyo/1999")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk()) // Verifica que la respuesta es 200 OK
                .andExpect(jsonPath("$.length()").value(3))  // Verifica que la lista tiene 3 elementos (solo los juegos de 1999)
